...
void Random::playerPowerPlantAuctionStrategy(Player* player) {

}
void Random::playerBuyingRessourcesStrategy(Player* player) {

}
void Random::playerBuildingStrategy(Player* player) {

}
void Random::playerBureaucracyStrategy(Player* player) {

	bool isGood = false;

	//Notifies the strategy of the user on the console (in this case: RANDOM) :

	cout << "\n" << player->getName() << " strategy is at RANDOM" << endl;
	
	/*
	For this assignment, the random strategy is very basic: 
	It only generates a random number and stores it into the integer value: randomIndex. 
	This will randomly decide whether or not the user player will power his or her cities. 
	*/

	//int randomIndex = rand() % 100; // takes a random number.

	//If the random generated number is greater than 75 and the size of the player's city possession is > 0, 
	// then the user can power his or her cities:
	...
	//if the random generator was < 75 or if the size of the player's power plant cards were <= 0: 
	else {
		cout << "You are not powering this turn" << endl; 
	}
